...
SOAP_SOURCE_STAMP("@(#) soapC.c ver 2.8.117 2024-02-08 00:54:30 GMT")
...
SOAP_FMAC3 struct ns__hello * SOAP_FMAC4 soap_in_ns__hello(struct soap *soap, const char *tag, struct ns__hello *a, const char *type)
{
	if (soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
	(void)type; /* appease -Wall -Werror */
	a = (struct ns__hello*)soap_id_enter(soap, soap->id, a, SOAP_TYPE_ns__hello, sizeof(struct ns__hello), NULL, NULL, NULL, NULL);
	if (!a)
		return NULL;
	...
	{	a = (struct ns__hello *)soap_id_forward(soap, soap->href, (void*)a, 0, SOAP_TYPE_ns__hello, SOAP_TYPE_ns__hello, sizeof(struct ns__hello), 0, NULL, NULL);
		if (soap->body && soap_element_end_in(soap, tag))
			return NULL;
	}
	return a;
...
{
	struct ns__hello *p;
	struct ns__hello *a = (struct ns__hello*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct ns__hello));
	for (p = a; p && n--; p++)
